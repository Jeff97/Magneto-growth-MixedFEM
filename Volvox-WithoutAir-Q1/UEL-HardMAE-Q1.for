...
! ---------------------------------------------------------------------
      ...
      !  GloUVar(col1, col2, col3)
      !   col1 - element pointer
      !   col2 - integration point pointer
      !   col3 - SDV pointer

      !  NELEMENT
      !   Total number of elements in the real mesh.

      !  ELEMOFFSET
      !   Offset of dummy mesh.  
...
!
     ...
     & Potential_Mech, Potential_Mag, PowerMagni
...
!       Lambda = Em*Nu/(1.0+Nu)/(1-2.0*Nu)
      ...
      ! loading control
      TotalT = 1.0 
      IF (Cur_time .GT. 2.0*TotalT) THEN
...
!      &    Nphi, dNphi_dX, dNphi_dY, dNphi_dZ, Jac_phi)
  ...
  ! prescribe magnetic field TODO: ------------------------------------
      ! distribution of magnetization vector Mr, for 3D bendng 
        PowerMagni = PROPS(4)*Sqrt((GaussXc-0.5)**2+(GaussYc-0.5)**2)
     &               /(Sqrt((GaussXc-0.5)**2+(GaussYc-0.5)**2))


      IF (Cur_time .GT. TotalT) THEN
        ! assign magnetization vector Mr
        MRef(1) = PowerMagni*(GaussXc-0.5)
        MRef(2) = PowerMagni*(GaussYc-0.5)
        MRef(3) = PowerMagni*1.0
      END IF

      IF (Cur_time .LE. TotalT) THEN
        MRef(1) = PowerMagni*(GaussXc-0.5)*Cur_time
        MRef(2) = PowerMagni*(GaussYc-0.5)*Cur_time
        MRef(3) = PowerMagni*1.0*Cur_time
      END IF
...
!      &     DetA, Ib_A, b_A, Mu0, dphi, Im)
  ...
  ! TODO: pass stress components to global variable
     ...
     &                              + (dphi(2)**2) + (dphi(3)**2))
        GloUVar(JELEM,gp,14) = MRef(1)
        GloUVar(JELEM,gp,15) = MRef(2)
...
! ----------------------------- GrowthFun -----------------------------
     ...
     1     GF_xCoord,GF_yCoord,GF_zCoord, GF_time,TotalT,g)
      ...
      DOUBLE PRECISION :: TotalT, GF_time, Pi, g,  R, alpha
      INTEGER :: gp
      ...
      ! If (GF_time .GT. 3) THEN
      !    alpha = 0.7 + 0.6*(GF_time - 3)/12.0
      ! END IF

      Lambda1z0 = (4*R*alpha)/(2*R**2+(1+2*(-1+GF_xCoord)*GF_xCoord
     &               +2*(-1+GF_yCoord)*GF_yCoord)*alpha**2)
      Lambda1z1 = -((4*R*alpha)/(2*R**2+(1-2*GF_xCoord+2*GF_xCoord**2
     &               -2*GF_yCoord+2*GF_yCoord**2)*alpha**2))
      Lambda2z0 = Lambda1z0
      Lambda2z1 = Lambda1z1
      ...
      Lambda3 = 2.0
      ! I cannot get total time from UEL variable, please modify TotalT manually 
