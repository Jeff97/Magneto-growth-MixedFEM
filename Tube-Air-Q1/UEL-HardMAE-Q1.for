...
! Parameters input ----------------------------------------------------
      ...
      ! IF (MatFlag < 0.5) THEN ! this is air 
      !   Em = PROPS(1) * (1.01 - Cur_time)
      ! END IF
...
!       Lambda = Em*Nu/(1.0+Nu)/(1-2.0*Nu)
      ...
      IF (Cur_time .GT. 2.0*TotalT) THEN
        ...
        BExt(1) = PROPS(6)*(Cur_time-3.0)
        BExt(2) = PROPS(7)*(Cur_time-3.0)
        BExt(3) = PROPS(8)*(Cur_time-3.0)
      ...
      ELSE IF (Cur_time .GT. TotalT) THEN
        ...
        MRef(3) = PROPS(5)
        ! assign external magnetic induction vector Ba
        BExt(1) = PROPS(6)*(1.0-Cur_time)
        BExt(2) = PROPS(7)*(1.0-Cur_time)
        BExt(3) = PROPS(8)*(1.0-Cur_time)
...
!      &    Nphi, dNphi_dX, dNphi_dY, dNphi_dZ, Jac_phi)
  ...
  ! Growth functions input --------------------------------------------
        ...
        ! TODO: growth only happens in 1st stage 
...
!      &     DetA, Ib_A, b_A, Mu0, dphi, Im)
  ...
  ! TODO: pass stress components to global variable
        ...
        ! GloUVar(JELEM,gp,13) = -dphi(1)
        ! GloUVar(JELEM,gp,14) = -dphi(2)
        ! GloUVar(JELEM,gp,15) = -dphi(3)
  ...
  ! difference of volume
        ...
        ! WRITE(*,*) "gp, DetG, DetF, dvol0 =", gp, DetG, DetF, dvol0
        ! WRITE(*,*) "gp, DetF, Pressure =", gp, DetF, Pressure
     ...
     &                  - b2*CauchySS(8) - b3*CauchySS(9)
        ...
        ! TODO: check this residual vector 2, pressure Res_phi is minus F_phi
